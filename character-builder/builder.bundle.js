...
async function syncGlobalEntries(){try{const response=await fetch(`${getApiBase()}/prompt-entries/global`,{headers:{'Content-Type':'application/json',},});if(!response.ok){console.warn('PortraitPrompt: Global entries fetch failed with status',response.status);return null;}
const apiEntries=await response.json();if(!Array.isArray(apiEntries)){console.warn('PortraitPrompt: Global entries API returned non-array');return null;}
return apiEntries;}catch(e){console.warn('PortraitPrompt: Global entries fetch error',e);return null;}}
async function syncFromAPI(){if(apiSyncAttempted)return;apiSyncAttempted=true;if(isAuthenticated()){const token=getAuthToken();if(!token)return;try{const response=await fetch(`${getApiBase()}/prompt-entries`,{headers:{'Authorization':`Bearer ${token}`,'Content-Type':'application/json',},});if(!response.ok){console.warn('PortraitPrompt: API fetch failed with status',response.status);return;}
...
adminCache=parseEntriesToCache(apiEntries);console.log('PortraitPrompt: Loaded',apiEntries.length,'entries from API (cloud)');}catch(e){console.warn('PortraitPrompt: API fetch error',e);}}else{const globalEntries=await syncGlobalEntries();if(globalEntries&&globalEntries.length>0){adminCache=parseEntriesToCache(globalEntries);console.log('PortraitPrompt: Loaded',globalEntries.length,'global entries for demo mode');}}}
