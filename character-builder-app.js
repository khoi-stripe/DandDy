...
const App = (window.App = {
  ...
  async showAbilities(question) {
    ...
    narratorPanel.insertAdjacentHTML(
      ...
      <div class="question-card" data-question-id="${question.id}">
        <div class="options-container">
          <label class="settings-label" style="margin-bottom: 4px;">Ability generation method:</label>
          <div style="display: flex; align-items: center; gap: 8px; max-width: 420px;">
            <select id="ability-method-select" class="input-field" style="flex: 1;">
              ${optionsHTML}
            </select>
            <button class="button-primary" style="margin-top: 0; white-space: nowrap;" onclick="App.handleAbilityFromSelect()">
              > ROLL
            </button>
          </div>
        </div>
  ...
  async handleAbilityMethod(method) {
    ...
    const narratorPanel = document.getElementById('narrator-panel');
    narratorPanel.innerHTML += Components.renderNarratorMessage(
      `Your abilities have been determined. HP: ${hitPoints}. Good luck with that.`,
    );
    Utils.scrollToBottom(true);
  ...
  async openSettings() {
    ...
    document
      .getElementById('demo-mode-checkbox')
      .addEventListener('change', (e) => {
        StorageService.setDemoModeEnabled(e.target.checked);
      });
  },
  ...
  saveSettings() {
    ...
    if (apiKeyInput) {
      StorageService.setAPIKey(apiKeyInput.value);
    }
    this.closeSettings();
  },
  ...
  async generateCustomAIPortrait() {
    ...
    if (!StorageService.getAPIKey()) {
      this.showSystemMessage(
        'OpenAI API key is not set in settings. Add one to generate custom AI portraits.',
      );
      return;
    }
  ...
  openPromptModal(character) {
    const defaultPrompt = AIService.buildPortraitPrompt
      ...
      <div class="prompt-modal-overlay" id="prompt-modal-overlay">
        <div class="prompt-modal" onclick="event.stopPropagation();">
          <div class="prompt-modal-header">
            <span>✨ Customize AI Portrait Prompt</span>
            <button class="prompt-modal-close" onclick="App.closePromptModal(false)">×</button>
          </div>
          
          <div class="prompt-modal-instructions">
            Edit the prompt below to guide DALL-E 3 in generating your character's portrait.
...
[ GENERATING CUSTOM AI PORTRAIT... ]
  ...
  showSystemMessage(text) {
    ...
    narratorPanel.insertAdjacentHTML(
      'beforeend',
      Components.renderNarratorMessage(`[ SYSTEM ] ${text}`),
    );
    Utils.scrollToBottom(true);
  },
