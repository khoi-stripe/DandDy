...
function renderSpellTagsInContainer(container, spells, level) {
    ...
    const spellTagsHtml = spells.map(spellName => `<span class="spell-tag">${Utils.escapeHtml(spellName)}<span class="spell-tag-remove"onclick="removeSpellFromEdit('${Utils.escapeHtml(spellName.replace(/'/g, "\\'"))}','${level}')">&times;</span></span>`).join('');
...
function showSpellLevelUpPrompt(character, spellInfo) {
    ...
    const modalHtml = `<div id="spellLevelUpModal"class="modal show"><div class="modal-content"><div class="modal-header"><h2 class="modal-title">NEW SPELLS AVAILABLE</h2><button class="modal-close"onclick="closeSpellLevelUpPrompt()">&times;</button></div><div class="modal-body"><p class="terminal-text"style="white-space: pre-line;">${Utils.escapeHtml(message)}</p><p class="terminal-text-small terminal-text-dim"style="margin-top: 1rem;">Select a spell level to add spells,or close this dialog to add them later via the Edit menu.</p></div><div class="modal-footer"style="flex-wrap: wrap; gap: 0.5rem;">${spellLevelButtons}<button class="terminal-btn"onclick="closeSpellLevelUpPrompt()">LATER</button></div></div></div>`;
    ...
    openSpellPicker({
        ...
        onConfirm: async (selectedSpells) => {
            ...
            try {
                ...
                showNotification(`Added ${selectedSpells.length}spell${selectedSpells.length>1?'s':''}!`);}catch(error){console.error('Failed to add spells:',error);showNotification('Failed to add spells','error');}}});}
window.initializeSpellEditSection=initializeSpellEditSection;window.removeSpellFromEdit=removeSpellFromEdit;window.openSpellPickerForEdit=openSpellPickerForEdit;window.getEditSpells=getEditSpells;window.checkAndPromptForNewSpells=checkAndPromptForNewSpells;window.closeSpellLevelUpPrompt=closeSpellLevelUpPrompt;window.openSpellLevelUpPicker=openSpellLevelUpPicker;window.filterInlineSpellPicker=filterInlineSpellPicker;window.toggleInlineSpellSelection=toggleInlineSpellSelection;window.closeInlineSpellPicker=closeInlineSpellPicker;window.confirmInlineSpellSelection=confirmInlineSpellSelection;